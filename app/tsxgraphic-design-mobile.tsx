...
export const ParallaxScrollm = ({
  ...
  const translateSecond = useTransform(scrollYProgress, [0, 1], [10, 100]);
  ...
  const thirdPart = images.slice(3 * third);
        ...
        <div className="grid gap-10">
          {thirdPart.map((el, idx) => (
            <motion.div style={{ y: translateThird }} key={"grid-3" + idx}>
              <Image
                src={el}
                className="h-80 w-full object-cover object-center rounded-lg gap-50 !m-0 !p-0"
                height="400"
                width="400"
                alt="thumbnail"
              />
